...
if (!is_admin()) {
  ...
  // Project Category
    add_action('rest_api_init', 'show_project_category_in_rest');

    function show_project_category_in_rest()
    {
      // register_rest_field ( 'name-of-post-type', 'name-of-field-to-return', array-of-callbacks-and-schema() )
        register_rest_field('show_project', 'project-category', array(
        'get_callback' => 'get_project_category_for_api',
        'schema' => null,
        ));
    }

    function get_project_category_for_api()
    {
        global $post;
        $result = [];
